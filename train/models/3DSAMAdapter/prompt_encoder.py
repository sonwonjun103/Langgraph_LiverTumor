...
class TwoWayTransformer(nn.Module):
    ...
    def forward(
        self,
        image_embedding: Tensor, # (1, 256, 32, 32, 32)
        image_pe: Tensor, # (1, 256, 32, 32, 32)
        point_coord, # (1,1,1,30,3)
    ) -> Tuple[Tensor, Tensor]:
        """
        Args:
        ...
        # print(f"In two transformer : {image_embedding.shape} imagepe : {image_pe.shape}, point coord {point_coord.shape}")
        ...
        #print(f"in twoway transformer: {point_embedding.shape} {point_pe.shape}")
        # (1, 256, 30) (1, 256, 30)
        ...
        # print(f"after visalu sampler : {image_embedding.shape} {point_embedding.shape}")
        # Apply transformer blocks and final layernorm
        for layer in self.layers:
            image_embedding, point_embedding = layer(
                image_embedding,
                point_embedding, # = prompt embedding
                image_pe,
                point_pe,
            )
            # print(f"After attention: {image_embedding.shape} {point_embedding.shape}")
...
class TwoWayAttentionBlock(nn.Module):
    ...
    def forward(self, img_embed, point_embed, img_pe, point_pe) -> Tuple[Tensor, Tensor]:
        #print(f" global query : {self.global_query.shape}")
...
class PromptEncoder(nn.Module):
    ...
    def forward(
        ...
        """
        # print(image_embeddings.shape, point_coord.shape)
        # (1, 256, 32, 32, 32), (1, 30, 3) 
        ...
        # print(f"image embeddings: {image_embeddings.shape}, image_pe : {image_pe.shape} point coord: {point_coord.shape}")
        # (1, 256, 32, 32, 32) (1. 256, 32, 32, 32) (1, 1, 1, 30, 3)
        # print(f"prompt encoder min max : {np.min(point_coord)} {np.max(point_coord)}")
        features = self.transformer(image_embeddings, image_pe, point_coord)
        # print(f"features 1 : {features.shape}")
        # (1, 32768, 256)
        features = features.transpose(1,2).reshape([1, -1] + feat_size)
        # print(f"features 2 : {features.shape}")
        # (1, 256, 32, 32, 32)
        return features
    ...
    def get_img_pe(self, size: Tuple[int, int], device) -> torch.Tensor: # size = (32, 32, 32)
...
if __name__=='__main__':
    ...
    ie = torch.randn(1, 256, 32, 32, 32)
    point_coord = torch.rand(1,30 , 3)

    pred = prompt_encoder(ie, point_coord)