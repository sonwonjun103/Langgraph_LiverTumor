...
class Adapter(nn.Module):
    ...
    def forward(self, features):
        ...
        out = F.relu(out)
        #print(f"before conv shape : {out.shape}") 
        #1 32 32 32 384
        out = out.permute(0, 4, 1, 2, 3)
        #print(f"before conv shape : {out.shape}") 
        #1 384 32 32 32 
        out = self.conv(out)
        #print(f"after conv shape : {out.shape}") 
        #1 384 32 32 32 
        ...
        #print(f"After second liear : {out.shape}")
        # 1 32 32 32 768
        out = F.relu(out)
        out = features + out
...
class LayerNorm3d(nn.Module): # 여기도 훈련 가능한 weight와 bias가 있음
...
class ImageEncoderViT_3d_v2(nn.Module):
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        with torch.no_grad():
            ...
            # print(f"After 2d patch embedding: {x.shape}") （512, 32, 32, 768） 
        if self.num_slice > 1:
            x = self.slice_embed(x.permute(3, 1, 2, 0).unsqueeze(0)) # train
            # print(f"After 3d patch embedding : {x.shape}") (1, 768, 32, 32, 32)
            x = x.permute(0, 2, 3, 4, 1)
            # print(f"After 3d patch embedding reshape : {x.shape}") (1, 32, 32, 32, 768)
        else:
            x = x.permute(1, 2, 0, 3).unsqueeze(0)

        # print(f"init pos embed : {self.pos_embed.shape}, init depth embed : {self.depth_embed.shape}")
        if self.pos_embed is not None:
            pos_embed = F.avg_pool2d(self.pos_embed.permute(0,3,1,2), kernel_size=2).permute(0,2,3,1).unsqueeze(3)
            #print(f"After 2d position embedding : {pos_embed.shape} depth embedding : {self.depth_embed.shape}" ) 
            #(1, 32, 32, 1, 768)
            pos_embed = pos_embed + (self.depth_embed.unsqueeze(1).unsqueeze(1)) # train

            #print(f"After 3d position embedding (summation): {pos_embed.shape} depth embedding : {self.depth_embed.unsqueeze(1).unsqueeze(1).shape}") 
            #(1, 32, 32, 32, 768)
            #print(x.shape)
            x = x + pos_embed
            # print(f'Summation of original and pos embed : {x.shape}') (1, 32, 32, 32, 768)
        ...
        for blk in self.blocks[6:12]:
            ...
            if idx % 3 == 0 and idx != 12:
                feature_list.append(self.neck_3d[idx//3-1](x.permute(0, 4, 1, 2, 3)))
        # print(x.shape) (1, 32, 32, 32, 768)
        x = self.neck_3d[-1](x.permute(0, 4, 1, 2, 3))
        # print(x.shape) (1, 256, 32, 32, 32)
...
class ImageEncoderViT_3d(nn.Module):
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        with torch.no_grad():
            x = self.patch_embed(x)
            # print(f"After patch embed : {x.shape}") # (512 32 32 768)
        if self.num_slice > 1:
            x = self.slice_embed(x.permute(3, 1, 2, 0).unsqueeze(0))
            # print(f"After patch embed: {x.shape}") # (1 768 32 32 32)
            x = x.permute(0, 2, 3, 4, 1)
            # print(f"After patch embed: {x.shape}") # (1, 32 32 32 768)
        else:
            x = x.permute(1, 2, 0, 3).unsqueeze(0)
        ...
        if self.pos_embed is not None:
            pos_embed = F.avg_pool2d(self.pos_embed.permute(0,3,1,2), kernel_size=2).permute(0,2,3,1).unsqueeze(3)
            # print(f"pos embed : {pos_embed.shape} depth embed : {self.depth_embed.shape}")
            # (1, 32, 32, 1, 768), (1, 32, 768)
            pos_embed = pos_embed + (self.depth_embed.unsqueeze(1).unsqueeze(1))
            # print(f"pos embed : {pos_embed.shape} depth embed : {self.depth_embed.shape}")
            # (1, 32, 32, 32, 768), (1, 32, 768)
            x = x + pos_embed
            # print(f"after pos embed : {x.shape}") (1, 32, 32, 32, 768)
...
if __name__=='__main__':
    ...
    sample = torch.randn(512, 3, 512, 512)
    x, feature_list = model(sample)